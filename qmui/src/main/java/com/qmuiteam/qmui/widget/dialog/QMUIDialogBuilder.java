...
public abstract class QMUIDialogBuilder<T extends QMUIDialogBuilder> {
     ...
     * @param listener 点击回调事件
     */
    public T addCancelAction(CharSequence str, QMUIDialogAction.ActionListener listener) {
        return addAction(0, str, QMUIDialogAction.ACTION_PROP_NEGATIVE, listener);
    }
     ...
     * @param listener 点击回调事件
     */
    public T addCancelAction(int strRes, QMUIDialogAction.ActionListener listener) {
        return addAction(0, strRes, QMUIDialogAction.ACTION_PROP_NEGATIVE, listener);
    }
    ...
    public QMUIDialog create() {
        ...
        return create(R.style.BaseDialogStyle);
    }
//    /**
//     * 只产生一个 Dialog, 不显示出来
//     *
    ...
    protected void onCreateHandlerBar(final QMUIDialog dialog, ViewGroup parent, Context context) {
        ...
        if (size > 0) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (size == 2 && i == 1) {
                    actionView.onlyShowLeftDivider(4, 4, 4, ContextCompat.getColor(context, mActionDividerColorRes));
                }
